...
export const TestFilesServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const findAll = Effect.fn('TestFilesService.findAll')(function* (testId?: string) {
      if (testId !== undefined) {
    ...
    const update = Effect.fn('TestFilesService.update')(function* (
      ...
      const testFile = new TestFile(row)
      yield* Effect.logInfo('Test file updated').pipe(Effect.annotateLogs({ id }))
      return testFile
    })
    ...
    return { upload, findAll, findOne, update, download, presignedUrl, remove }
  }),
)
