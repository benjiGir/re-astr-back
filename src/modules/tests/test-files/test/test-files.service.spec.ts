...
import { mockTestFile, mockTestFiles, mockReportFile } from './test-files.service.mock';
...
describe('TestFilesService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw NotFoundException when test does not exist', async () => {
      // Arrange
      jest.spyOn(testsService, 'findOne').mockRejectedValue(
  ...
  describe('update', () => {
    ...
    it('should update a test file metadata successfully', async () => {
      // Arrange
      const updatedFile = { ...mockTestFile, fileType: 'report' as const, metadata: { updated: true } };
  ...
  describe('uploadFile', () => {
    ...
    it('should throw BadRequestException when upload fails', async () => {
      ...
      // Act & Assert
      await expect(
        service.uploadFile(mockMultipartFile, uploadDto, 'user-123'),
  ...
  describe('getPresignedUrl', () => {
    it('should generate presigned URL with default expiry', async () => {
      // Arrange
      const presignedUrl = 'https://minio.example.com/test-archives/tests/2024/01/file.png?expires=3600';
