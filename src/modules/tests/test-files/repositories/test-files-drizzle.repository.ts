...
import { testFiles, type TestFile, type NewTestFile } from '../../../../database/schema/test-files.schema';
...
export class TestFilesDrizzleRepository implements ITestFilesRepository {
  ...
  async create(data: NewTestFile): Promise<TestFile> {
    const [testFile] = await this.db.drizzle
      .insert(testFiles)
      .values(data)
      .returning();
  ...
  async findByTest(testId: string): Promise<TestFile[]> {
    return this.db.drizzle
      .select()
      .from(testFiles)
      .where(eq(testFiles.testId, testId));
  }
