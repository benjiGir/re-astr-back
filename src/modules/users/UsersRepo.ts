...
export const UsersRepoLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      findById: (id) =>
        Effect.map(db.select().from(users).where(eq(users.id, id)).limit(1), ([row]) => Option.fromNullishOr(row)),
      ...
      assignRole: (id, role) =>
        Effect.map(db.update(users).set({ role }).where(eq(users.id, id)).returning(), ([row]) =>
          Option.fromNullishOr(row),
        ),

      remove: (id) => Effect.asVoid(db.delete(users).where(eq(users.id, id))),
    }
  }),
)
