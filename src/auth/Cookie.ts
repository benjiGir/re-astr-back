...
export const sign = (token: string): Effect.Effect<string, never, SessionConfig> =>
  Effect.map(SessionConfig, ({ cookieSecret }) => `${token}.${signature(Redacted.value(cookieSecret), token)}`)

export const verify = (cookieValue: string): Effect.Effect<string, InvalidCookie, SessionConfig> =>
  Effect.gen(function* () {
