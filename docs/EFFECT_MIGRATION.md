...
2. **Décision produit tranchée avec l'utilisateur** : aucun envoi d'email réel (l'ancienne config Better Auth n'avait pas de callback `sendResetPassword`, et il n'y a de service mailer nulle part dans le code) — `forgot-password`/`reset-password` implémentent la mécanique du token de bout en bout (génération/stockage dans `verifications`/vérification/expiration 1h), mais le token ne sort jamais du serveur, comme avant.
3. `CredentialsRepo.ts` + `Credentials.ts` : découpage repo/service habituel (11 méthodes primitives — plus large que les repos CRUD classiques car `Credentials` orchestre 4 tables, pas une seule). `signUp` (userId cascade), `signIn` (échec **générique** `InvalidCredentials` que ce soit email inconnu ou mot de passe faux — pas d'énumération d'utilisateurs), `signOut`, `requestPasswordReset` (silencieux si email inconnu, même raison), `resetPassword` (**en plus** : invalide toutes les sessions existantes de l'utilisateur — une réinitialisation de mot de passe est généralement liée à une suspicion de compromission).
...
- **Poser/retirer un cookie depuis un handler (Phase 5)** : `HttpApiBuilder.securitySetCookie(security, value, options)` enregistre un hook pré-réponse qui pose le cookie — prend le **même** descripteur `HttpApiSecurity.apiKey(...)` que celui utilisé pour le *lire* dans un middleware, donc autant l'exporter une seule fois et le partager (`Cookie.sessionCookieSecurity`) plutôt que dupliquer la clé du cookie en dur à deux endroits. Pour le retirer (sign-out), pas d'équivalent `securityRemoveCookie` — construire la réponse normalement (`HttpServerResponse.json(...)`, qui renvoie un `Effect`, pas une valeur brute) puis `.pipe(HttpServerResponse.removeCookie(name))` dessus, et la renvoyer directement depuis le handler (cf. le point Phase 4 sur `SuccessType | HttpServerResponse`).
- **Un seul `HttpApiGroup` ne peut pas mélanger endpoints publics et protégés (Phase 5)** : `.middleware(X)` s'applique à **tous** les endpoints du groupe, pas un sous-ensemble. `sign-up`/`sign-in`/`forgot-password`/`reset-password` (pas de cookie à ce stade) et `sign-out`/`get-session` (cookie requis) doivent donc être deux `HttpApiGroup` séparés (noms différents, tous les deux ajoutés à l'`Api`), le second seul avec `.middleware(Authorization)` — même si conceptuellement ils forment "un seul" module `auth`.
- **Élargir un service partagé plutôt que dupliquer une requête (Phase 5)** : `sign-out`/`get-session` ont besoin de la session **entière** (id/token/expiresAt/ipAddress/userAgent), pas juste de l'identité. Plutôt qu'une requête DB séparée dans ces deux handlers, `CurrentUser` (et le `SELECT` fait par `Authorization`) a été élargi pour la porter — la requête tournait déjà pour chaque requête authentifiée, sélectionner plus de colonnes ne coûte rien de plus, et ça évite de dupliquer la logique de résolution session→user ailleurs.
